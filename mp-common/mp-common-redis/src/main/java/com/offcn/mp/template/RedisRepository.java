...
//            log.info("[redisTemplate redis]放入 缓存  url:{} ========缓存时间为{}秒", key, time);
     ...
     * @param time  过期时间
     */
    public void setPExpire(final String key, final String value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            byte[] keys = serializer.serialize(key);
            byte[] values = serializer.serialize(value);
...
//            log.info("[redisTemplate redis]放入 缓存  url:{} ========缓存时间为{}秒", key, time);
    ...
    /**
     * 一次性添加数组到   过期时间的  缓存，不用多次连接，节省开销 单位s
     *
...
//                log.info("[redisTemplate redis]放入 缓存  url:{} ========缓存时间为:{}秒", keys[i], time);
     ...
     * @return long
     */
    public Long getRemainingExpire(String key){
...
//        log.info("[redisTemplate redis]  getHashValue()  key={}", key);
    ...
    public long hIncr(final String key,final String field) {
        return redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> redisSerializer = getRedisSerializer();
            return connection.hIncrBy(redisSerializer.serialize(key),redisSerializer.serialize(field),1L);
        });
    }
